...
class Train_3M:
    ...
    def train(self):
        ...
        for epoch in tqdm(range(self.epochs)):
            ...
            matrix1 = results['emb_bnm_omics1']
            matrix1 = matrix1.to(torch.float32)
            _, s1, _ = torch.svd(matrix1)

            loss_bnm1 = - torch.sum(s1) / matrix1.size(0)
           
            matrix2 = results['emb_bnm_omics2']
            matrix2 = matrix2.to(torch.float32)
            _, s2, _ = torch.svd(matrix2)

            loss_bnm2 = - torch.sum(s2) / matrix2.size(0)

            matrix3 = results['emb_bnm_omics3']
            matrix3 = matrix3.to(torch.float32)
            
            _, s3, _ = torch.svd(matrix3)

            loss_bnm3 = - torch.sum(s3) / matrix3.size(0)

            bnm_loss = (self.weight_factors[0] * loss_bnm1 + self.weight_factors[1] * loss_bnm2 + \
                   self.weight_factors[2] * loss_bnm3)
            ...
            loss = bnm_loss + dpcl_loss + loss_adj
            # loss = bnm_loss + dpcl_loss

            print(
                f"[Epoch {epoch}]"
                f"bnm Loss: {bnm_loss:.4f} | "
                f"dpcl Loss: {dpcl_loss:.4f} | "
                f"loss adj: {loss_adj:.4f}"
            )
