...
def clr_normalize_each_cell(adata, inplace=True):
    ...
    adata.X = np.apply_along_axis(
        ...
        seurat_clr, 1, (adata.X.A if scipy.sparse.issparse(adata.X) else np.array(adata.X.copy()))
    )
    return adata
...
def sparse_mx_to_torch_sparse_tensor(sparse_mx):
    ...
    return torch.sparse_coo_tensor(indices, values, shape, dtype=torch.float32, device='cuda:1')
