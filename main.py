...
import argparse
import time
from RIA.preprocess import fix_seed
from RIA.preprocess import clr_normalize_each_cell, pca, lsi
from RIA.preprocess import construct_neighbor_graph as construct_neighbor_graph_2M
from RIA.preprocess_3M import construct_neighbor_graph as construct_neighbor_graph_3M
from RIA.Train_model import Train
from RIA.Train_model_3M import Train_3M
from RIA.utils import clustering


def main(args):
    # This is a placeholder for the ArgumentParser logic in the original script
    # In a real run, `args` would be populated from the command line.
    pass
    ...
    # read data
    if args.data_type in ['10x', 'Stereo-CITE-seq']:
        ...
        adata_omics1 = sc.read_h5ad(args.file_fold + 'adata_RNA.h5ad')
        adata_omics2 = sc.read_h5ad(args.file_fold + 'adata_Pro.h5ad')
    ...
    if args.data_type == '10x':
        ...
        data = construct_neighbor_graph_2M(adata_omics1, adata_omics2, datatype=args.data_type, Arg=args)
    elif args.data_type == 'Spatial-epigenome-transcriptome':
        # RNA
        sc.pp.filter_genes(adata_omics1, min_cells=10)
    ...
    adata.obsm['RIA'] = output['RIA'].copy()

    # Clustering
    tool = 'mclust'  # mclust, leiden, and louvain
    clustering(adata, key='RIA', add_key='RIA', n_clusters=args.n_clusters, method=tool, use_pca=True)
    ...
    sc.pp.neighbors(adata, use_rep='RIA', n_neighbors=500)
    sc.tl.umap(adata)

    # We remove the title from the initial plot call, to set it manually later.
    sc.pl.umap(adata, color='RIA', ax=ax_list[0], title='', s=spot_size, show=False)
    sc.pl.embedding(adata, basis='spatial', color='RIA', ax=ax_list[1], title='', s=spot_size, show=False)

    # ==================== FONT SIZE ADJUSTMENT AREA ====================
    # 在这里修改您需要的字体大小
    TITLE_FONTSIZE = 18  # "RIA" 标题的字号
    AXIS_LABEL_FONTSIZE = 14  # "UMAP1", "spatial1" 等坐标轴标签的字号
    # 图例中数字的大小由scanpy默认控制，这里我们不去修改它

    # 应用字体大小到第一个图 (UMAP)
    ax_list[0].set_title('RIA', fontsize=TITLE_FONTSIZE)
    ax_list[0].set_xlabel('UMAP1', fontsize=AXIS_LABEL_FONTSIZE)
    ax_list[0].set_ylabel('UMAP2', fontsize=AXIS_LABEL_FONTSIZE)

    # 应用字体大小到第二个图 (Spatial)
    ax_list[1].set_title('RIA', fontsize=TITLE_FONTSIZE)
    ax_list[1].set_xlabel('spatial1', fontsize=AXIS_LABEL_FONTSIZE)
    ax_list[1].set_ylabel('spatial2', fontsize=AXIS_LABEL_FONTSIZE)
    # =================================================================
